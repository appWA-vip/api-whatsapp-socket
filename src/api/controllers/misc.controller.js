...
exports.downProfile = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key]?.DownloadProfile(req.query.id);
    return res.status(201).json({ error: false, data: data });
};

exports.getStatus = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key]?.getUserStatus(req.query.id);
    return res.status(201).json({ error: false, data: data });
};
...
exports.blockUser = async (req, res) => {
    ...
    } else return res.status(201).json({ error: false, message: 'Contact Unblocked' });
};
...
exports.getUserOrGroupById = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key].getUserOrGroupById(req.query.id);
    return res.status(201).json({ error: false, data: data });
};
