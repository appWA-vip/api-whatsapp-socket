...
exports.makeAdmin = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key].makeAdmin(req.body.id, req.body.users);
    return res.status(201).json({ error: false, data: data });
};

exports.demoteAdmin = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key].demoteAdmin(req.body.id, req.body.users);
    return res.status(201).json({ error: false, data: data });
};

exports.listAll = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key].getAllGroups(req.query.key);
    return res.status(201).json({ error: false, data: data });
};
...
exports.getInviteCodeGroup = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key].getInviteCodeGroup(req.query.id);
    return res.status(201).json({ error: false, link: 'https://chat.whatsapp.com/' + data });
};

exports.getInstanceInviteCodeGroup = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key].getInstanceInviteCodeGroup(req.query.id);
    return res.status(201).json({ error: false, link: 'https://chat.whatsapp.com/' + data });
};
...
exports.groupInviteInfo = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key].groupGetInviteInfo(req.body.code);
    return res.status(201).json({ error: false, data: data });
};

exports.groupJoin = async (req, res) => {
    const data = await WhatsAppInstances[req.query.key].groupAcceptInvite(req.body.code);
    return res.status(201).json({ error: false, data: data });
};
